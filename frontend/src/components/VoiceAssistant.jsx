...
const VoiceAssistant = () => {
  ...
  const processAudio = async (audioBlob) => {
    ...
    try {
      ...
      // Transcribe audio
      const transcriptionResponse = await axios.post('/api/voice/transcribe', formData, {
        headers: {
      ...
      if (transcriptionResponse.data.success) {
        ...
        if (questionResponse.data.success) {
          setAiResponse(questionResponse.data.answer)
        }
      }
    } catch (err) {
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-700">
                <strong>Tip:</strong> Ask questions about your GTU subjects like "Explain polymorphism in C++" or "What is database normalization?"
              </p>
            </div>
          </div>
        </div>
